...
export class CommentController {
    ...
    @UseGuards(jwtGuard)
    @Delete(':id')
    async deleteCmt(@User('id') userId: number, @Param('id', ParseIntPipe) commentId: number) {
        if (await this.tool.havePermisson(userId, commentId, 'comment')) return this.service.deleteCmt(commentId);
        else throw new HttpException("you don't own this comment", HttpStatus.FORBIDDEN);
    }
}
